...
class PMMTrainer():
    ...
    def _train_epoch(self, model):
        ...
        with torch.autograd.set_detect_anomaly(True):
            for batch_pressure_images, batch_depth_images, batch_labels, batch_pmap, _, _ in iter(self.train_loader):
                self.opt.zero_grad()
    ...
    def _validate_epoch(self, model):
        ...
        running_losses = defaultdict(float)
        with torch.no_grad():
            for batch_pressure_images, batch_depth_images, batch_labels, batch_pmap, batch_verts, _ in iter(self.val_loader):
